...
/**
 * Single fetch for open pipeline leads + activities (aging + manager risk).
 */
export function usePipelineHealthData(organizationId, assignedToFilter = null) {
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase
      .from('leads')
      .select('id, name, score, category, status, created_at, urgency, source, responded, assigned_to')
  ...
  return { leads, activitiesByLeadId, managerInsights, loading, reload: load }
}
